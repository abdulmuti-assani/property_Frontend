...
const Properties = () => {
  ...
  //  to toggle wishlist (optimistic: flip the heart immediately, revert on failure)
  ...
  // فلترة الـ dummy data محليًا عشان الفلاتر تفضل شغالة برضو من غير باك اند
  const applyLocalFilters = (list, currentFilters) => {
    let result = [...list];
    ...
    if (currentFilters.propertyType.length > 0) {
      result = result.filter((p) =>
        currentFilters.propertyType.includes(
          (p.propertyType || "").toLowerCase(),
        ),
      );
    }
    if (currentFilters.bhk) {
      if (currentFilters.bhk === "5+") {
        result = result.filter((p) => Number(p.bhk) >= 5);
      } else {
        result = result.filter((p) => String(p.bhk) === currentFilters.bhk);
      }
    }
    if (currentFilters.maxPrice) {
      result = result.filter((p) => (p.price || 0) <= currentFilters.maxPrice);
    }
    if (currentFilters.furnishing && currentFilters.furnishing.length > 0) {
      result = result.filter((p) =>
        currentFilters.furnishing.includes(p.furnishing),
      );
    }
  ...
  const fetchProperties = async (currentFilters) => {
    try {
      ...
      // الرد ممكن يكون { properties: [...] } أو array مباشرة
      ...
      if (!data) {
        throw new Error("Unexpected response shape from /api/property");
      }
      ...
      console.warn(
        "No backend available or unexpected response — using dummy data instead.",
        err,
      );
      setProperties(applyLocalFilters(dummyProperties, currentFilters));
      setError(null);
    } finally {
      setLoading(false);
    }
